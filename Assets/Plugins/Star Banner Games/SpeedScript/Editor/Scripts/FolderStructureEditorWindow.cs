...
namespace SBG.SpeedScript
{
    public class FolderStructureEditorWindow : EditorWindow
	{
        ...
        private void OnGUI()
        {
            ...
            //SELECT/SETUP/DELETE FOLDER STRUCTURE
            EditorGUILayout.BeginHorizontal();
            ...
            if (GUILayout.Button("Setup")) TrySetupFolderStructure();
            ...
            //EXPORT STRUCTURE
            if (GUILayout.Button("Export Project Structure...", GUILayout.MinHeight(18))) OpenStructureCreationWindow();
            //OPEN SOURCE
            if (GUILayout.Button("Open Source Folder", GUILayout.MaxHeight(18))) TryOpenSourceFolder();

            //CLEAN EMPTY FOLDERS
            EditorGUILayout.Space(15);
            if (GUILayout.Button("Clean Empty Folders", GUILayout.MinHeight(30))) TryCleanEmptyFolders();
        }
        ...
        private void CreateSubFolders(string projectFolder, string[] folderNames)
        {
            ...
            for (int i = 0; i < folderNames.Length; i++)
            {
                ...
                string path = $"{rootPath}/{splitName[0]}";
                ...
                for (int j = 0; j < splitName.Length; j++)
                {
                    ...
                    {
                        AssetDatabase.CreateFolder(rootPath, splitName[j]);
                    }
                }
            }
